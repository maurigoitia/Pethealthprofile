...
export function ActionTray() {
  ...
  const getPriorityColor = (priority: "high" | "medium" | "low") => {
    switch (priority) {
      case "high":
        return "bg-[#D32F2F]"; // Plano High Priority (Red)
      case "medium":
        return "bg-[#ED6C02]"; // Plano Medium Priority (Amber)
      case "low":
        return "bg-[#1A9B7D]"; // Plano Low Priority (Emerald)
    }
  };
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 text-left min-w-0">
                    ...
                    {isAiProjection ? (
                      <p className="text-[9px] text-[#7C3AED] dark:text-[#A78BFA] font-bold uppercase tracking-wider mt-1 flex items-center gap-1">
                        <MaterialIcon name="auto_awesome" className="text-xs" />
                        Detectado automáticamente — pendiente confirmación
                      </p>
                    ) : action.autoGenerated && (
                      <p className="text-[9px] text-[#1A9B7D] dark:text-[#10B981] font-bold uppercase tracking-wider mt-1">

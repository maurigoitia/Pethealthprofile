  ...
  return (
    <div className="min-h-screen" style={{ background: "#F0FAF9", fontFamily: "'Manrope',sans-serif" }}>
      <div className="max-w-md mx-auto pb-24">
        ...
        <div className="px-5 mb-4"><div className="relative"><span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true" style={{ fontSize: "20px" }}>search</span><input type="text" placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 rounded-[12px] bg-white border border-slate-200 text-sm focus:ring-2 focus:ring-[#074738] outline-none shadow-[0_2px_8px_rgba(0,0,0,0.04)]" /></div></div>
        <div className="px-5 space-y-3">
          ...
          : filtered.length === 0 ? <div className="bg-white rounded-[16px] p-8 text-center border border-[rgba(0,0,0,0.04)]"><span className="material-symbols-outlined text-slate-300 mb-3" aria-hidden="true" style={{ fontSize: "48px" }}>pets</span><h3 className="font-bold text-slate-900 mb-1">{search ? "Sin resultados" : "Sin pacientes"}</h3><p className="text-slate-500 text-sm">{search ? "Probá otro término." : "Aparecerán cuando registres consultas."}</p></div>
          : filtered.map(p => (
            <button key={p.id} onClick={() => onSelectConsultation(p.id)} className="w-full bg-white rounded-[16px] p-4 text-left border border-[rgba(0,0,0,0.04)] shadow-[0_2px_8px_rgba(0,0,0,0.04)] active:translate-y-[-1px] transition-all duration-150">
              <div className="flex items-center gap-3">
                <div className="size-12 rounded-full bg-[#E0F2F1] flex items-center justify-center"><span className="material-symbols-outlined text-[#074738]" aria-hidden="true" style={{ fontSize: "24px" }}>pets</span></div>

...
export function VetLoginScreen() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault(); setResetError(""); setResetSuccess(""); setResetLoading(true);
    try { await sendPasswordResetEmail(auth, resetEmail.trim().toLowerCase()); setResetSuccess("Revisá tu correo."); }
    catch { setResetError("No se pudo enviar el correo."); }
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          <div className="size-16 rounded-full bg-white/10 flex items-center justify-center mb-4">
            <span className="material-symbols-outlined text-white" style={{ fontSize: "32px" }}>stethoscope</span>
          </div>
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          ...
          <div className="flex justify-end"><button type="button" onClick={() => { setShowReset(true); setResetEmail(email); }} className="text-white/60 text-xs font-semibold">¿Olvidaste tu contraseña?</button></div>
      ...
      {showReset && (
        <>
          ...
          <div className="fixed inset-x-4 bottom-8 z-50 bg-white rounded-[24px] shadow-2xl p-6 max-w-md mx-auto">
            <h2 className="text-lg font-black text-slate-900 mb-4">Recuperar contraseña</h2>
            <form onSubmit={handleResetPassword} className="space-y-3">
              <input type="email" placeholder="Tu correo" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} className="w-full px-4 py-3 rounded-[12px] border border-slate-200 text-sm focus:ring-2 focus:ring-[#074738] outline-none" required />
              {resetError && <p className="text-red-600 text-xs font-semibold text-center">{resetError}</p>}
              {resetSuccess ? <p className="text-emerald-700 text-xs font-semibold text-center bg-emerald-50 rounded-xl p-3">{resetSuccess}</p> : <button type="submit" disabled={resetLoading} className="w-full py-3 rounded-[12px] bg-[#074738] text-white font-bold text-sm disabled:opacity-60">{resetLoading ? "Enviando..." : "Enviar link"}</button>}

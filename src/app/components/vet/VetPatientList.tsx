    ...
    <div className="min-h-screen" style={{ background: "#F0FAF9", fontFamily: "'Manrope',sans-serif" }}>
      <div className="max-w-md mx-auto pb-24">
        ...
        <div className="px-5 space-y-3">
          ...
          : filtered.length === 0 ? <div className="bg-white rounded-[16px] p-8 text-center border border-[rgba(0,0,0,0.04)]"><span className="material-symbols-outlined text-slate-300 mb-3" style={{ fontSize: "48px" }}>pets</span><h3 className="font-bold text-slate-900 mb-1">{search ? "Sin resultados" : "Sin pacientes"}</h3><p className="text-slate-500 text-sm">{search ? "Probá otro término." : "Aparecerán cuando registres consultas."}</p></div>
          : filtered.map(p => (

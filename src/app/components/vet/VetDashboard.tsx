  ...
  return (
    <div className="min-h-screen" style={{background:"#F0FAF9",fontFamily:"'Manrope',sans-serif"}}><div className="max-w-md mx-auto pb-24">
      <div className="px-5 pt-12 pb-6 rounded-b-[28px]" style={{background:"linear-gradient(135deg,#074738 0%,#0a6b54 100%)"}}>
        <div className="flex items-center justify-between mb-5"><div><p className="text-white/70 text-sm">Bienvenido</p><h1 className="text-2xl font-black text-white" style={{fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Dr. {name}</h1></div>
          <div className="flex items-center gap-2">{vp?.verified && <span className="px-2 py-1 rounded-full bg-emerald-400/20 text-emerald-300 text-xs font-bold flex items-center gap-1"><span className="material-symbols-outlined" style={{fontSize:"14px"}}>verified</span>Verificado</span>}<div className="size-11 rounded-full bg-white/15 flex items-center justify-center"><span className="material-symbols-outlined text-white" style={{fontSize:"22px"}}>stethoscope</span></div></div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white/10 rounded-[16px] p-3 text-center"><p className="text-2xl font-black text-white">{stats.today}</p><p className="text-white/60 text-xs font-semibold">Hoy</p></div>
          <div className="bg-white/10 rounded-[16px] p-3 text-center"><p className="text-2xl font-black text-[#1A9B7D]">{stats.pending}</p><p className="text-white/60 text-xs font-semibold">Pendientes</p></div>
          <div className="bg-white/10 rounded-[16px] p-3 text-center"><p className="text-2xl font-black text-white">{stats.patients}</p><p className="text-white/60 text-xs font-semibold">Pacientes</p></div>
        </div>
      </div>
      <div className="px-5 mt-5"><div className="grid grid-cols-2 gap-3">
        <button onClick={()=>setTab("patients")} className="bg-white rounded-[16px] p-4 text-left shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-slate-100"><span className="material-symbols-outlined text-[#074738] mb-2" style={{fontSize:"28px"}}>pets</span><p className="font-bold text-slate-900 text-sm">Mis pacientes</p><p className="text-slate-500 text-xs mt-0.5">Ver historial</p></button>
        <button onClick={()=>navigate("/vet/new-consultation")} className="bg-[#074738] rounded-[16px] p-4 text-left shadow-[0_4px_12px_rgba(7,71,56,0.3)]"><span className="material-symbols-outlined text-[#1A9B7D] mb-2" style={{fontSize:"28px"}}>add_circle</span><p className="font-bold text-white text-sm">Nueva consulta</p><p className="text-white/60 text-xs mt-0.5">Registrar atención</p></button>
      </div></div>
      <div className="px-5 mt-6"><div className="flex items-center justify-between mb-3"><h2 className="text-lg font-black text-slate-900" style={{fontFamily:"'Plus Jakarta Sans',sans-serif"}}>Consultas recientes</h2><button onClick={()=>setTab("patients")} className="text-[#074738] text-sm font-bold">Ver todas</button></div>
        ...
        : cons.length===0 ? <div className="bg-white rounded-[16px] p-8 text-center border border-slate-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)]"><span className="material-symbols-outlined text-slate-300 mb-3" style={{fontSize:"48px"}}>clinical_notes</span><h3 className="font-bold text-slate-900 mb-1">Sin consultas aún</h3><p className="text-slate-500 text-sm mb-4">Comenzá registrando tu primera consulta.</p><button onClick={()=>navigate("/vet/new-consultation")} className="px-5 py-2.5 rounded-[12px] bg-[#074738] text-white font-bold text-sm">Crear consulta</button></div>
        : <div className="space-y-3">{cons.map(c=>(
          <button key={c.id} onClick={()=>setSelCon(c.id)} className="w-full bg-white rounded-[16px] p-4 text-left border border-slate-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
            <div className="flex items-start justify-between"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className="material-symbols-outlined text-[#074738]" style={{fontSize:"18px"}}>pets</span><p className="font-bold text-slate-900 text-sm">{c.petName}</p></div><p className="text-slate-500 text-xs">Tutor: {c.tutorName}</p><p className="text-slate-600 text-xs mt-1">{c.reason}</p></div><span className={`px-2 py-1 rounded-full text-[10px] font-bold ${sc[c.status]}`}>{sl[c.status]}</span></div>
          </button>
        ))}</div>}
      </div>

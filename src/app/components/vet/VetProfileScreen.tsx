  ...
  return (
    <div className="min-h-screen" style={{background:"#F0FAF9",fontFamily:"'Manrope',sans-serif"}}><div className="max-w-md mx-auto pb-24">
      ...
      <div className="px-5 space-y-4">
        <div className="bg-white rounded-[16px] p-5 text-center border border-[rgba(0,0,0,0.04)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <div className="size-20 mx-auto rounded-full bg-[#E0F2F1] flex items-center justify-center mb-3 overflow-hidden">{userPhoto?<img src={userPhoto} alt="" className="w-full h-full object-cover"/>:<span className="material-symbols-outlined text-[#074738]" aria-hidden="true" style={{fontSize:"36px"}}>stethoscope</span>}</div>
          ...
          {profile?.verified && <span className="inline-flex items-center gap-1 px-3 py-1 mt-2 rounded-full bg-emerald-50 text-emerald-700 text-xs font-bold"><span className="material-symbols-outlined" aria-hidden="true" style={{fontSize:"14px"}}>verified</span> Verificado</span>}
        </div>
        <div className="bg-white rounded-[16px] p-4 border border-[rgba(0,0,0,0.04)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]"><h3 className="text-xs font-bold text-[#074738] uppercase tracking-wide mb-2">Info profesional</h3><Row l="Matrícula" v={profile?.matricula}/><Row l="Especialidad" v={profile?.specialty}/><Row l="Email" v={profile?.email||user?.email||""}/><Row l="Teléfono" v={profile?.phone}/></div>
        {profile?.clinicName && <div className="bg-white rounded-[16px] p-4 border border-[rgba(0,0,0,0.04)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]"><h3 className="text-xs font-bold text-[#074738] uppercase tracking-wide mb-2">Clínica</h3><Row l="Nombre" v={profile.clinicName}/></div>}
        <button onClick={()=>navigate("/login")} className="w-full bg-white rounded-[16px] p-4 text-left border border-[rgba(0,0,0,0.04)] shadow-[0_2px_8px_rgba(0,0,0,0.04)] flex items-center gap-3 active:translate-y-[-1px] transition-all duration-150"><span className="material-symbols-outlined text-[#074738]" aria-hidden="true" style={{fontSize:"24px"}}>pets</span><div><p className="font-bold text-slate-900 text-sm">¿Tenés mascota?</p><p className="text-slate-500 text-xs">Ingresá a Pessy como tutor</p></div></button>

/**
 * Clinical AI functions extracted from clinicalIngestion.ts
 * (Strangler Fig refactoring).
 *
 * Covers: Gemini API calls, OCR via Gemini, clinical extraction prompt
 * building, AI-based classification & extraction, heuristic fallbacks,
 * veterinary evidence derivation, and AI response parsing / normalisation.
 */
...
import type {
  AttachmentMetadata,
  ClinicalClassificationInput,
  ClinicalClassificationOutput,
...
import {
  ...
  hasStrongNonClinicalSignal,
  hasStrongVeterinaryEvidence,
  isTrustedClinicalSender,
  isVetDomain,
} from "./textProcessing";

// ---------------------------------------------------------------------------
// Session-metric callback type
// ---------------------------------------------------------------------------

/**
 * Optional callback to record per-stage metrics into the ingestion session.
 * Decouples this module from the Firestore session-management layer so
 * it can be tested and compiled in isolation.
 */
export type SessionMetricRecorder = (args: {
  sessionId: string;
...
// ---------------------------------------------------------------------------
// Veterinary evidence derivation
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Veterinary evidence priority override
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AI response -> ClinicalExtractionOutput
// ---------------------------------------------------------------------------
  ...
  const detectedEvents: ClinicalEventExtraction[] = eventsRaw
    .map((item: unknown) => {
      ...
      const normalizedMeds = meds
        ...
        .filter((value: ClinicalMedication | null): value is ClinicalMedication => Boolean(value));
      const normalizedLabs = labs
        ...
        .filter((value: ClinicalLabResult | null): value is ClinicalLabResult => Boolean(value));
    ...
    .filter((value: ClinicalEventExtraction | null): value is ClinicalEventExtraction => Boolean(value))
    .map((event: ClinicalEventExtraction) => {
...
export function heuristicClinicalClassification(input: ClinicalClassificationInput): ClinicalClassificationOutput {
  ...
  const hasVeterinaryEvidenceResult = hasStrongVeterinaryEvidence(input);
  const isClinical =
    (keywordRegex.test(normalized) || hasClinicalAttachment || hasVeterinaryEvidenceResult) &&
    !(!hasClinicalAttachment && (hasNoise || hasHumanHealthcareNoise) && !hasVeterinaryEvidenceResult);
  return {
...
export async function classifyClinicalContentWithAi(
  ...
  if (hasHumanHealthcareNoise && !hasVeterinaryEvidenceResult && !hasClinicalAttachment) {
  ...
  try {
    ...
    const jsonResult = asRecord(parsed);
    if (sessionId && onMetric) {
      await onMetric({
        sessionId,
    ...
    return {
      is_clinical: jsonResult.is_clinical === true,
      confidence: clamp(asNonNegativeNumber(jsonResult.confidence, 0), 0, 100),
    };
  } catch (error) {
...
// ---------------------------------------------------------------------------
// AI-based clinical event extraction
// ---------------------------------------------------------------------------
...
export async function extractClinicalEventsWithAi(args: {
  ...
  try {
    ...
    for (const chunk of chunks) {
      ...
      const started = Date.now();
      let aiCallsUsed = 1;
      let response = await callGemini(geminiPayload, CLINICAL_AI_TIMEOUT_MS);
      ...
      if (!parsed) {
        aiCallsUsed += 1;
        response = await callGemini(geminiPayload, CLINICAL_AI_TIMEOUT_MS);
        rawText = extractGeminiText(response);
        parsed = tryParseJson(rawText);
      }

      if (args.sessionId && args.onMetric) {
        await args.onMetric({

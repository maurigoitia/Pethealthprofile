...
function collectUidCandidates(value: unknown): string[] {
...
function normalizeBase64(raw: string): string {
  ...
  if (cleaned.startsWith("data:")) {
    ...
    if (commaIdx >= 0) return cleaned.slice(commaIdx + 1);
  }
  return cleaned;
}
...
function inferContentType(fileName: string, providedType: string): string {
  ...
  if (ALLOWED_CONTENT_TYPES.has(loweredType)) return loweredType;
...
async function assertPetAccess(petId: string, uid: string): Promise<void> {
  ...
  const sharedAccessByUid = data.sharedAccessByUid && typeof data.sharedAccessByUid === "object"
    ? data.sharedAccessByUid as Record<string, unknown>
    : {};
...
export const uploadPetPhoto = functions
  .runWith({ timeoutSeconds: 60, memory: "512MB" })
  .region("us-central1")
  .https.onCall(async (rawData: UploadPetPhotoInput, context): Promise<UploadPetPhotoResult> => {
    ...
    if (uid && typeof rawData?.uid === "string" && rawData.uid.trim() && rawData.uid.trim() !== uid) {
      console.warn("[uploadPetPhoto] uid mismatch payload/auth", {
        authUid: uid,
        payloadUid: rawData.uid.trim(),
        petId: typeof rawData?.petId === "string" ? rawData.petId.trim() : "",
      });
    }
    ...
    const fileName = sanitizeFileName(typeof rawData?.fileName === "string" ? rawData.fileName : "pet_photo.jpg");
    const base64 = normalizeBase64(typeof rawData?.base64 === "string" ? rawData.base64 : "");
    if (!base64) {
    ...
    const contentType = inferContentType(fileName, typeof rawData?.contentType === "string" ? rawData.contentType : "");

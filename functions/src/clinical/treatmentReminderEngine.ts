...
type DoseAction = "given" | "given_late" | "missed" | "skipped" | "snoozed";
...
type TreatmentLike = {
  ...
  isChronic: boolean;
  timezone: string;
  sourceCollection: "medications" | "treatments";
};
...
function parseLocalTimeOrDefault(value: unknown): { hour: number; minute: number } {
...
function parseLocalDateTimeToUtc(dateKey: string, hour: number, minute: number, timeZone: string): Date {
...
function parseFrequencyHours(raw: unknown): number | null {
  const text = normalizeText(raw);
  if (!text) return null;

  if (/(cronico|indefin|continu)/.test(text)) return 24;
  ...
  if (/diario|cada 24/.test(text)) return 24;
  if (/semanal|cada 7 dias/.test(text)) return 24 * 7;
  if (/mensual|cada 30 dias/.test(text)) return 24 * 30;

  return null;
}

function parseDosageMagnitude(raw: unknown): number | null {
...
function computeDosageSimilarity(aRaw: string, bRaw: string): number {
...
function computeClinicScore(a: string, b: string): number {
  const aa = normalizeText(a);
  const bb = normalizeText(b);
  if (!aa || !bb) return 0;
  if (aa === bb) return 100;
...
function computeDedupeScore(candidate: {
  medicationName: string;
  startDateIso: string;
  dosage: string;
  clinic: string;
...
async function resolvePetName(petId: string): Promise<string> {
  ...
  if (!petSnap.exists) return "tu mascota";
  const data = asRecord(petSnap.data());
  return (typeof data.name === "string" && data.name.trim()) ? data.name.trim() : "tu mascota";
}

async function canAccessPet(uid: string, petId: string): Promise<boolean> {
  const petSnap = await db().collection("pets").doc(petId).get();
  if (!petSnap.exists) return false;
  const data = asRecord(petSnap.data());
  const ownerId = typeof data.ownerId === "string" ? data.ownerId : "";
  const coTutorUids = Array.isArray(data.coTutorUids)
    ? data.coTutorUids.filter((x) => typeof x === "string") as string[]
    : [];
  return ownerId === uid || coTutorUids.includes(uid);
}

function extractTreatmentFromMedication(
  docId: string,
  ...
  if (!userId || !petId || !medicationName.trim()) return null;
...
function extractTreatmentFromTreatmentDoc(
  docId: string,
  data: Record<string, any>,
  timezone: string,
): TreatmentLike | null {
...
async function cancelPendingByTreatment(treatmentId: string): Promise<void> {
...
async function scheduleFromTreatment(treatment: TreatmentLike): Promise<void> {
  ...
  const horizonDays = treatment.isChronic
    ? 14
    : Math.min(MAX_GENERATION_HORIZON_DAYS, Math.max(1, Math.ceil(((end?.getTime() || now.getTime()) - now.getTime()) / ONE_DAY_MS)));
  ...
  while (nextAt.getTime() <= horizonEnd.getTime() && generated < MAX_REMINDERS_PER_TREATMENT) {
    if (end && nextAt.getTime() > end.getTime()) break;
...
async function listUserTokens(userId: string): Promise<Array<{ id: string; token: string }>> {
...
async function recomputeTreatmentMetrics(treatmentId: string): Promise<void> {
  ...
  eventsSnap.docs.forEach((docSnap) => {
    const status = String(docSnap.data()?.status || "pending") as DoseEventStatus;
    if (status === "given" || status === "given_late") taken += 1;
...
export const onMedicationWriteScheduleV3 = functions
  .region("us-central1")
  .firestore.document("medications/{medicationId}")
  .onWrite(async (change, context) => {
    const medicationId = context.params.medicationId as string;
...
export const onTreatmentWriteScheduleV3 = functions
  .region("us-central1")
  .firestore.document("treatments/{treatmentId}")
  .onWrite(async (change, context) => {
    const treatmentId = context.params.treatmentId as string;
    if (!change.after.exists) {
...
export const dispatchTreatmentRemindersV3 = functions
  ...
  .onRun(async () => {
    ...
    for (const docSnap of snap.docs) {
      const data = asRecord(docSnap.data());
      const status = String(data.status || "pending") as ReminderStatus;
      if (status !== "pending") continue;

      const scheduledMs = parseIsoMs(data.scheduledFor);
      if (!scheduledMs || scheduledMs > now + 5 * ONE_MINUTE_MS) continue;
...
export const markMissedTreatmentDosesV3 = functions
  .region("us-central1")
  ...
  .onRun(async () => {
    ...
    for (const docSnap of snap.docs) {
      const data = asRecord(docSnap.data());
      const status = String(data.status || "pending") as DoseEventStatus;
      if (status !== "pending") continue;
...
export const recordDoseEventV3 = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    ...
    const snoozeMinutes = Number(data?.snoozeMinutes || 30);
    ...
    if (action === "snoozed") {
      ...
      batch.set(db().collection("dose_events").doc(newDoseEventId), {
        ...dose,
        id: newDoseEventId,
        ...
        updatedAt: nowIso,
        createdAt: nowIso,
        originalDoseEventId: doseEventId,
      }, { merge: true });
...
export const evaluateTreatmentDedupV3 = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    if (!context.auth?.uid) {
      throw new functions.https.HttpsError("unauthenticated", "Debes iniciar sesión.");
    ...
    const existingList: Record<string, any>[] = Array.isArray(data?.existing)
      ? data.existing.map((x: unknown) => asRecord(x))
      : [];
    ...
    const scored = existingList.map((item: Record<string, any>) => {
      const model = {
    ...
    return {
      ...
      top: scored[0] || null,
      all: scored,
      hasPotentialDuplicate: scored.some((row: { score: number }) => row.score >= 80),
    };
  });

export const syncTreatmentTimezoneV3 = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    const uid = context.auth?.uid;
    if (!uid) {

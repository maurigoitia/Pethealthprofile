...
interface EpisodeMedication {
  name: string;
  dosage: string | null;
  frequency: string | null;
}

interface EpisodeSourceEvent {
  id: string;
  title: string;
  documentType: string | null;
  date: string | null;
  source: "gmail" | "scanner" | "manual";
}
...
interface EpisodeSeed {
  ...
  sourceEvents: EpisodeSourceEvent[];
  confidence: number;
...
interface BackfillResult {
  total_medical_events_scanned: number;
  eligible_medical_events: number;
  total_appointments_scanned: number;
  recent_episodes_written: number;
  historical_episodes_written: number;
  buckets_written: number;
  profile_snapshots_written: number;
  appointments_projected: number;
...
function confidenceFromLabel(value: unknown): number {
  const normalized = normalize(value);
  if (normalized === "high" || normalized === "alta" || normalized === "confirmado") return 0.92;
  if (normalized === "medium" || normalized === "media") return 0.8;
  if (normalized === "low" || normalized === "baja") return 0.65;
  return 0.45;
}

function inferEventConfidence(row: Record<string, unknown>): number {
  const overall = asNumber(row.overallConfidence);
  if (overall !== null) {
    if (overall > 1) return Math.max(0, Math.min(1, overall / 100));
...
function extractEventDate(row: Record<string, unknown>): string | null {
  const extracted = asRecord(row.extractedData);
  return (
    asString(extracted.eventDate) ||
...
function isStudyDocument(documentType: string, extracted: Record<string, unknown>): boolean {
  return (
    ["xray", "lab_test", "echocardiogram", "electrocardiogram"].includes(documentType) ||
    /(radiografia|ecografia|ecg|laboratorio|lab|koh|microscopia|rx|imagen|radiologico)/.test(
      normalize(extracted.studyType || extracted.suggestedTitle || extracted.observations || "")
    )
  );
}

function hasAppointmentLanguage(value: string): boolean {
  return /(turno|consulta|recordatorio|confirmacion|confirmado|agendad|programad|reprogramad|cancelad|cita)/.test(
    normalize(value)
  );
}

function isLikelyPrescription(documentType: string, extracted: Record<string, unknown>): boolean {
  const medications = asArray<Record<string, unknown>>(extracted.medications);
...
function classifyEpisodeType(row: Record<string, unknown>): EpisodeType {
  const extracted = asRecord(row.extractedData);
  ...
  if (documentType === "vaccine") return "vaccination";
  if (isLikelyPrescription(documentType, extracted)) return "prescription";
  if (documentType === "appointment" || hasAppointmentLanguage(titleText)) return "appointment";
...
function primaryStudyLabel(row: Record<string, unknown>): string | null {
  const extracted = asRecord(row.extractedData);
  return (
    cleanLabel(extracted.studyType) ||
...
function extractMedications(row: Record<string, unknown>): EpisodeMedication[] {
  const extracted = asRecord(row.extractedData);
  return asArray<Record<string, unknown>>(extracted.medications)
    .map((medication) => ({
...
function inferSource(row: Record<string, unknown>): "gmail" | "scanner" | "manual" {
  const extracted = asRecord(row.extractedData);
  if (asString(extracted.sourceSender) || asString(extracted.sourceSubject) || asString(extracted.sourceReceivedAt)) return "gmail";
  if (asBoolean(row.ocrProcessed) || asString(row.fileType) === "pdf" || asString(row.fileType) === "image") return "scanner";
  return "manual";
}

function buildSourceEvent(row: Record<string, unknown>, id: string): EpisodeSourceEvent {
  const extracted = asRecord(row.extractedData);
  return {
    id,
...
function buildEpisodeSeedFromEvent(row: Record<string, unknown>, id: string): EpisodeSeed | null {
  const extracted = asRecord(row.extractedData);
  ...
  const specialty = cleanLabel((asArray<Record<string, unknown>>(extracted.detectedAppointments)[0] || {}).specialty) || null;
...
function mergeEpisodeSeeds(args: {
  ...
  const medicationsMap = new Map<string, EpisodeMedication>();
  ...
  const episodeType =
    sorted.every((seed) => seed.episodeType === "appointment")
      ? "appointment"
  ...
  const headline = (() => {
    ...
    if (episodeType === "prescription") {
      return medications[0]?.name ? `Tratamiento – ${medications[0].name}` : "Tratamiento indicado";
    }
  ...
  return {
    ...
    sourceEventIds,
    sourceEvents,
    sourceAppointmentIds: [] as string[],
...
function looksLikeClinicalAppointment(title: string, provider: string | null, clinic: string | null, date: string): boolean {
...
function buildEpisodeFromAppointment(args: {
  ...
  if (!looksLikeClinicalAppointment(title, provider, clinic, date)) return null;
...
function buildBucketSummary(episodes: Array<Record<string, unknown>>, petName: string, periodLabel: string): Record<string, unknown> | null {
  if (episodes.length <= 10) return null;
  const diagnoses = unique(episodes.flatMap((episode) => asArray<string>(episode.diagnoses))).slice(0, 3);
  const medications = unique(
    episodes.flatMap((episode) =>
      asArray<Record<string, unknown>>(episode.medications).map((medication) => cleanLabel(medication.name))
    )
  ).slice(0, 3);
  return {
...
function buildAnnualSummary(episodes: Array<Record<string, unknown>>, petName: string, yearKey: string): Record<string, unknown> {
  const diagnoses = unique(episodes.flatMap((episode) => asArray<string>(episode.diagnoses))).slice(0, 3);
  const medications = unique(
    episodes.flatMap((episode) =>
      asArray<Record<string, unknown>>(episode.medications).map((medication) => cleanLabel(medication.name))
...
function isQaUserAllowed(email: string): boolean {
...
async function deleteExistingDerivedDocs(args: { uid: string; petId: string }) {
  const deleted = { episodes: 0, buckets: 0, profiles: 0 };
  for (const collectionName of ["clinical_episodes", "clinical_episode_buckets"] as const) {
    ...
    await batch.commit();
    if (collectionName === "clinical_episodes") deleted.episodes += count;
    else deleted.buckets += count;
  }
...
function buildProfileSnapshot(args: {
  ...
  const allergyCandidates = unique([
    ...
    ...args.episodes.flatMap((episode) => asArray<string>(episode.diagnoses)),
  ])
...
async function maybeProjectAppointmentFromEpisode(episode: Record<string, unknown>): Promise<boolean> {
  if (asString(episode.episodeType) !== "appointment") return false;
  if (asString(episode.status) !== "confirmed") return false;
  ...
  await ref.set({
    ...
    sourceEventId: asArray<string>(episode.sourceEventIds)[0] || null,
...
async function runClinicalEpisodeBackfill(args: {
  ...
  if (!petSnap.exists) throw new Error("pet_not_found");
  ...
  const seedsByGroup = new Map<string, EpisodeSeed[]>();
  for (const document of eventsSnap.docs) {
    ...
    if (!seed) continue;
    if (seed.status === "needs_clean_upload") continue;
    result.eligible_medical_events += 1;
    const groupKey = buildGroupKey(seed);
    if (!seedsByGroup.has(groupKey)) seedsByGroup.set(groupKey, []);
    seedsByGroup.get(groupKey)!.push(seed);
  }

  const episodeDocs = Array.from(seedsByGroup.entries()).map(([groupKey, seeds]) =>
    mergeEpisodeSeeds({
      uid: args.uid,
      petId: args.petId,
      petName,
  ...
  for (const document of appointmentsSnap.docs) {
    ...
    if (!episode) continue;
    const duplicate = episodeDocs.some((candidate) => asArray<string>(candidate.sourceAppointmentIds).includes(document.id));
    if (!duplicate) episodeDocs.push(episode);
  }
  ...
  const yearEpisodes = new Map<string, Array<Record<string, unknown>>>();
  for (const episode of episodeDocs) {
    ...
    asArray<string>(bucket.episodeIds).push(asString(episode.id));
    bucket.episodeCount = asArray<string>(bucket.episodeIds).length;
    bucketDocs.set(period.bucketId, bucket);

    if (!yearEpisodes.has(period.yearKey)) yearEpisodes.set(period.yearKey, []);
  ...
  for (const [bucketId, bucket] of bucketDocs.entries()) {
    if (asString(bucket.periodType) === "month") {
      const bucketEpisodes = episodeDocs.filter((episode) => asArray<string>(bucket.episodeIds).includes(asString(episode.id)));
      bucket.bucketSummary = buildBucketSummary(bucketEpisodes, petName, asString(bucket.periodLabel));
    }
    if (result.sample_bucket_ids.length < 10) result.sample_bucket_ids.push(bucketId);
  }
  ...
  for (const [yearKey, episodes] of yearEpisodes.entries()) {
    ...
    if (result.sample_bucket_ids.length < 10) result.sample_bucket_ids.push(yearBucketId);
  }
  ...
  if (!dryRun) {
    ...
    for (const episode of episodeDocs) {
      ...
      if (result.sample_episode_ids.length < 10) result.sample_episode_ids.push(asString(episode.id));
...
export const backfillClinicalEpisodes = functions
  .runWith({
    timeoutSeconds: 540,
    memory: "1GB",

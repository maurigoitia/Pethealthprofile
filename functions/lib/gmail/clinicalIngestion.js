...
exports.ingestClinicalEmailWebhook = exports.forceRunEmailClinicalIngestion = exports.runEmailClinicalAiWorker = exports.runEmailClinicalAttachmentWorker = exports.runEmailClinicalScanWorker = exports.runEmailClinicalIngestionQueue = exports.triggerEmailClinicalIngestion = void 0;
...
const CLINICAL_AI_TIMEOUT_MS = 15000;
const CLASSIFICATION_AI_TIMEOUT_MS = 7000;
...
const MAX_EXTERNAL_LINK_TEXT_CHARS = 120000;
...
function getMaxExternalLinksPerEmail() {
    return getBoundedIntFromEnv("GMAIL_MAX_EXTERNAL_LINKS_PER_EMAIL", DEFAULT_MAX_EXTERNAL_LINKS_PER_EMAIL, 0, MAX_EXTERNAL_LINKS_PER_EMAIL_HARD_CAP);
}
function getExternalLinkFetchTimeoutMs() {
    return getBoundedIntFromEnv("GMAIL_EXTERNAL_LINK_FETCH_TIMEOUT_MS", DEFAULT_EXTERNAL_LINK_FETCH_TIMEOUT_MS, 2000, 20000);
}
function getExternalLinkMaxBytes() {
    return getBoundedIntFromEnv("GMAIL_EXTERNAL_LINK_MAX_BYTES", DEFAULT_EXTERNAL_LINK_MAX_BYTES, 100000, 20 * 1024 * 1024);
}
function getExternalLinkMaxRedirects() {
    return getBoundedIntFromEnv("GMAIL_EXTERNAL_LINK_MAX_REDIRECTS", DEFAULT_EXTERNAL_LINK_MAX_REDIRECTS, 0, MAX_EXTERNAL_LINK_REDIRECTS_HARD_CAP);
}
...
async function uploadAttachmentBase64ToStorage(args) {
    ...
    try {
        ...
        await (0, promises_1.pipeline)(stream_1.Readable.from(iterateBase64Chunks(normalizedBase64), { encoding: "ascii" }), createBase64DecodeTransform(), file.createWriteStream({
            resumable: false,
            metadata: {
...
function extractBodyText(payload) {
    ...
    for (const part of parts) {
        ...
        const data = asString((_a = part.body) === null || _a === void 0 ? void 0 : _a.data);
        if (!data)
            continue;
...
function getHeader(payload, headerName) {
    ...
    for (const header of headers) {
        if (asString(header.name).toLowerCase() === wanted)
...
function toClinicalOutput(json) {
    if (!json) {
        return {
...
function heuristicClinicalClassification(input) {
    const normalized = normalizeForHash([input.subject || "", input.fromEmail || "", input.bodyText || ""].filter(Boolean).join("\n"));
...
async function classifyClinicalContentWithAi(input, sessionId) {
...
async function fetchAttachmentMetadata(args) {
    ...
    const attachments = allParts
        .filter((part) => { var _a; return Boolean(asString(part.filename) || asString((_a = part.body) === null || _a === void 0 ? void 0 : _a.attachmentId)); })
    ...
    for (const part of attachments) {
        ...
        const attachmentId = asString((_a = part.body) === null || _a === void 0 ? void 0 : _a.attachmentId);
        const sizeBytes = asNonNegativeNumber((_b = part.body) === null || _b === void 0 ? void 0 : _b.size, 0);
...
async function extractDocxTextFromBase64(base64DataOrUrl) {
    ...
    return asString(result.value).slice(0, 40000);
}
    ...
    for (const part of attachments) {
        ...
        try {
            ...
            if (!data) {
                ...
                if (isPlainText) {
                    extractedText = decodeBase64UrlToText(data).slice(0, 40000);
...
async function maybeFinalizeSession(sessionId) {
    ...
    const totalActionableEvents = asNonNegativeNumber(counters.new_medical_events_created, 0) + asNonNegativeNumber(counters.events_requiring_review, 0);
    ...
    await admin.firestore().collection("user_email_config").doc(uid).set({
        ...
        documents_processed: asNonNegativeNumber(counters.new_medical_events_created, 0) +
...
async function closeStuckSessionAsPartial(args) {
    const nowIso = getNowIso();
    const counters = asRecord(args.sessionData.counters);
...
async function processAttachmentQueueJob(doc) {
    ...
    const planType = (asString(config.plan_type) === "premium" ? "premium" : "free");
    const sessionSnap = await admin.firestore().collection("gmail_ingestion_sessions").doc(sessionId).get();
    const sessionData = asRecord(sessionSnap.data());
    const counters = asRecord(sessionData.counters);
...
async function processAiQueueJob(doc) {
    ...
    const mode = (asString(payload.mode) === "extract" ? "extract" : "classify");
    const sessionId = asString(data.session_id);
    const uid = asString(data.user_id);
    if (!messageId || !rawDocId || !sessionId || !uid) {
        await markJobCompleted(doc.ref);
    ...
    const externalLinkRequiresLogin = externalLinkExtraction.metadata.some((row) => row.login_required === true || /login_required/i.test(asString(row.reason)));
    ...
    for (const event of clinical.detected_events) {
        ...
        const missingDoseInTreatment = event.event_type === "treatment" &&
        ...
        const requiresReview = clinical.confidence_overall < autoIngestThreshold ||
            event.confidence_score < autoIngestThreshold ||
            clinical.requires_human_review ||
            externalLinkRequiresLogin ||
            missingDoseInTreatment;
        if (requiresReview) {
            const reviewReason = clinical.reason_if_review_needed ||
            ...
            await mirrorBrainResolution({
                ...
                sourceMetadata: {
                    ...
                    ui_hint: {
                        image_fragment_url: ((_a = selectBestAttachmentForReview(attachmentMetadata)) === null || _a === void 0 ? void 0 : _a.storage_signed_url) || null,
                        source_file_name: ((_b = selectBestAttachmentForReview(attachmentMetadata)) === null || _b === void 0 ? void 0 : _b.filename) || null,
                    },
                },
            });
            await storeKnowledgeSignal({
        ...
        await mirrorBrainResolution({
            ...
            sourceMetadata: {
                ...
                ui_hint: {
                    source_file_name: ((_c = selectBestAttachmentForReview(attachmentMetadata)) === null || _c === void 0 ? void 0 : _c.filename) || null,
                    image_fragment_url: ((_d = selectBestAttachmentForReview(attachmentMetadata)) === null || _d === void 0 ? void 0 : _d.storage_signed_url) || null,
                },
            },
        });
        await storeKnowledgeSignal({
...
function toMedicalEventDocumentType(eventType) {
    if (eventType === "visit")
        return "appointment";
    ...
    if (eventType === "vaccination")
        return "vaccine";
    if (eventType === "diagnostic" || eventType === "imaging")
        return "lab_test";
    return "other";
}
...
function buildIncompleteTreatmentSubtitle(event) {
    const incomplete = event.medications
        ...
        const missing = [];
        if (!asString(med.dose))
            missing.push("dosis");
        if (!asString(med.frequency))
...
async function upsertClinicalReviewDraft(args) {
    ...
    const sourceStorageUri = asString(selectedAttachment === null || selectedAttachment === void 0 ? void 0 : selectedAttachment.storage_uri) || null;
    const sourceStoragePath = asString(selectedAttachment === null || selectedAttachment === void 0 ? void 0 : selectedAttachment.storage_path) || null;
    const sourceStorageSignedUrl = asString(selectedAttachment === null || selectedAttachment === void 0 ? void 0 : selectedAttachment.storage_signed_url) || null;
    const sourceFileName = asString(selectedAttachment === null || selectedAttachment === void 0 ? void 0 : selectedAttachment.filename) || null;
    const sourceMimeType = asString(selectedAttachment === null || selectedAttachment === void 0 ? void 0 : selectedAttachment.mimetype) || null;
...
async function upsertIncompleteTreatmentPendingAction(args) {
    ...
    const nowIso = getNowIso();
    const sourceStorageUri = asString((_a = args.sourceAttachment) === null || _a === void 0 ? void 0 : _a.storage_uri) || null;
    const sourceStoragePath = asString((_b = args.sourceAttachment) === null || _b === void 0 ? void 0 : _b.storage_path) || null;
    const sourceStorageSignedUrl = asString((_c = args.sourceAttachment) === null || _c === void 0 ? void 0 : _c.storage_signed_url) || null;
    const sourceFileName = asString((_d = args.sourceAttachment) === null || _d === void 0 ? void 0 : _d.filename) || null;
    const sourceMimeType = asString((_e = args.sourceAttachment) === null || _e === void 0 ? void 0 : _e.mimetype) || null;
    const imageFragmentUrl = sourceStorageSignedUrl || sourceStorageUri || null;
...
function buildSyncReviewSubtitle(args) {
    const summary = asString(args.event.description_summary).slice(0, 140) ||
...
async function detectSemanticDuplicateCandidate(args) {
    ...
    for (const coll of candidateCollections) {
        ...
        for (const doc of snap.docs) {
            ...
            const existingMedication = asString(row.name) ||
                asString(row.treatment_name) ||
                extractedMedications ||
                "";
            const existingDiagnosis = asString(row[coll.diagnosisField]) ||
                asString(row.title) ||
...
async function mirrorBrainResolution(args) {
    try {
        await (0, brainResolver_1.resolveBrainOutput)({
            ...
            sourceMetadata: Object.assign(Object.assign({}, args.sourceMetadata), { source: asString(args.sourceMetadata.source) || "gmail", pet_id_hint: asString(args.petIdHint) || null }),
        });
    }
    catch (error) {
...
async function ingestEventToDomain(args) {
    ...
    const incompleteTreatmentMeds = args.event.event_type === "treatment"
    ...
    const canonicalEventId = `gmail_evt_${sha256(`${args.uid}_${args.sourceEmailId}_${args.event.event_type}_${eventDate}_${title}`).slice(0, 20)}`;
    ...
    await admin.firestore().collection("medical_events").doc(canonicalEventId).set({
        ...
        source: "email_import",
        source_email_id: args.sourceEmailId,
        domain_ingestion_type: args.event.event_type === "vaccination"
            ? "vaccination"
...
async function processSession(sessionId, options) {
    ...
    const currentCounters = asRecord(sessionData.counters);
    const counters = {
    ...
    const configuredCap = asNonNegativeNumber(sessionData.max_mails_per_sync, asNonNegativeNumber(config.max_mails_per_sync, getMaxMailsPerSync(planType)));
    ...
    while (processedInRun < options.maxEmailsToProcess && Date.now() - started < options.hardDeadlineMs) {
        ...
        for (const message of messages) {
            ...
            try {
                ...
                const dateIso = parseGmailDate(asString(getHeader(payload, "Date")) || new Date(Number(detail.internalDate || Date.now())).toISOString());
                ...
                const rawBase = [
                    normalizeForHash(bodyText).slice(0, 100000),
            ...
            if (processedSinceFlush >= 5) {
                ...
                await sessionRef.update(Object.assign({ updated_at: getNowIso() }, scanCountersPatch));
...
exports.triggerEmailClinicalIngestion = functions
    ...
    const preferredPetId = asString(data === null || data === void 0 ? void 0 : data.petId) || null;
...
async function pickPendingJobsForSession(args) {
    ...
    const staleDocs = snap.docs.filter((doc) => {
        const row = asRecord(doc.data());
        if (asString(row.status) !== "processing")
            return false;
...
exports.forceRunEmailClinicalIngestion = functions
    ...
    const drainTimeoutMs = clamp(asNonNegativeNumber(body.drainTimeoutMs, FORCE_DRAIN_MAX_WAIT_MS), 10000, 8 * 60 * 1000);
...
exports.ingestClinicalEmailWebhook = functions
    ...
    const bodyText = asString(message.text) ||

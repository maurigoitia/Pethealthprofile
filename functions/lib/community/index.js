...
exports.onLostPetReport = functions.firestore
    ...
    const petName = ((_a = report.petSnapshot) === null || _a === void 0 ? void 0 : _a.name) || "una mascota";
    const breed = ((_b = report.petSnapshot) === null || _b === void 0 ? void 0 : _b.breed) || "";
    try {
        // Get all users with FCM tokens
        const usersSnap = await getDb().collectionGroup("fcm_tokens").get();
        const tokensByUser = new Map();
        for (const doc of usersSnap.docs) {
            const userId = (_c = doc.ref.parent.parent) === null || _c === void 0 ? void 0 : _c.id;
            if (!userId || userId === report.reportedBy)
                continue;
        ...
        functions.logger.info(`Lost pet ${reportId}: sent ${result.successCount}/${nearbyTokens.length} push notifications`);
...
exports.onPetSighting = functions.firestore
    .document("lost_pet_sightings/{sightingId}")
    .onCreate(async (snap, context) => {
    var _a;
    const sighting = snap.data();
    if (!sighting || !sighting.lostPetReportId)
        return;
    try {
        ...
        const petName = ((_a = report.petSnapshot) === null || _a === void 0 ? void 0 : _a.name) || "tu mascota";
        ...
        if (loc && report.lastSeenLocation) {
            const dist = distanceKm(report.lastSeenLocation.lat, report.lastSeenLocation.lng, loc.lat, loc.lng);
...
function scoreLivingSpace(adopter, pet) {
    const size = pet.size || "medium";
    const space = adopter.livingSpace;
    if (size === "large" && space === "apartment_small")
        return 0.2;
    if (size === "large" && space === "apartment_large")
        return 0.5;
    if (size === "large" && space.startsWith("house"))
        return 1.0;
    if (size === "medium" && space === "apartment_small")
        return 0.5;
    if (size === "small")
        return space === "apartment_small" ? 0.8 : 1.0;
    return 0.7;
}
function scoreExperience(adopter, pet) {
    const exp = adopter.experience;
    const needs = pet.specialNeeds || false;
    if (needs && exp === "none")
        return 0.1;
    if (needs && exp === "beginner")
        return 0.3;
    if (needs)
        return exp === "expert" ? 1.0 : 0.7;
    if (exp === "none")
        return 0.5;
    return exp === "expert" ? 1.0 : 0.8;
}
function scoreOtherPets(adopter, pet) {
    var _a;
    if (!adopter.hasOtherPets)
        return 1.0;
    const friendly = (_a = pet.goodWithOtherPets) !== null && _a !== void 0 ? _a : true;
    return friendly ? 0.9 : 0.3;
}
function scoreActivity(adopter, pet) {
    const petEnergy = pet.energyLevel || "moderate";
    const adopterLevel = adopter.activityLevel;
    if (petEnergy === adopterLevel)
        return 1.0;
    if (petEnergy === "high" && adopterLevel === "low")
        return 0.2;
    if (petEnergy === "low" && adopterLevel === "high")
        return 0.5;
    return 0.7;
}
function scoreSchedule(adopter, pet) {
    const hours = adopter.hoursAlonePerDay;
    const species = pet.species || "dog";
    if (species === "cat")
        return hours <= 12 ? 1.0 : 0.5;
    if (hours <= 4)
        return 1.0;
    if (hours <= 6)
        return 0.8;

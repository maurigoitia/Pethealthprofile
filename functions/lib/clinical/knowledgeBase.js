...
exports.PESSY_CLINICAL_KNOWLEDGE_VERSION = void 0;
exports.resolveClinicalKnowledgeContext = resolveClinicalKnowledgeContext;
const admin = require("firebase-admin");
exports.PESSY_CLINICAL_KNOWLEDGE_VERSION = "pessy-clinical-kb-v1-2026-03-02";
const KNOWLEDGE_SECTIONS = [
    {
        ...
        body: "No inventar datos. Si falta evidencia: null. Priorizar fecha clínica principal. Si es turno: no forzar diagnóstico/hallazgos. " +
    ...
    },
    {
        ...
        body: "Canino core: CDV, CAV-1/2, CPV-2, Rabies. Felino core: FPV, FCV, FHV-1. " +
    ...
    },
    {
        ...
        body: "Displasia de cadera (OFA): normal (excellent/good/fair), borderline, displásica (mild/moderate/severe). " +
    ...
    },
    {
        ...
        body: "Hepatitis crónica canina (ACVIM 2019): ALT sensible para daño hepatocelular; ALKP y GGT para colestasis. " +
    ...
    },
    {
        ...
        body: "Dermatitis atópica: diagnóstico de exclusión (FAD, alergia alimentaria, ectoparásitos). " +
    ...
    },
    {
        ...
        body: "Interpretación orientativa: BUN/CREA altos pueden sugerir falla renal/deshidratación/obstrucción; ALT alta daño hepatocelular; ALB baja pérdida renal/hepática/GI. " +
    ...
    },
    {
        ...
        body: "Evaluar BCS/MCS en cada visita. Fórmula base: RER = 30 * kg + 70 (kcal/día). " +
    ...
    },
    {
        ...
        body: "Usar marco de estadiaje 3 Ps: prognostic, practical, pertinent. " +
    ...
    },
    {
        ...
        body: "Etapas: kitten (0-1), young adult (1-6), mature (7-10), senior (>10). " +
    ...
    },
    {
        ...
        body: "Intervenir en inapetencia antes de 3 días o <80% RER por >3 días. " +
    ...
    },
    {
        ...
        body: "Pruebas ELISA pueden confirmar seroconversión (CDV, CPV, CAV, FPV). " +
...
function scoreSection(section, queryNormalized) {
    if (!queryNormalized.trim())
        return section.priority;
    let score = section.priority;
    for (const keyword of section.keywords) {
        const normalizedKeyword = normalizeText(keyword);
        if (queryNormalized.includes(normalizedKeyword))
            score += 25;
    }
    return score;
}
function pickKnowledgeSections(query, maxSections) {
    const queryNormalized = normalizeText(query || "");
    const sorted = [...KNOWLEDGE_SECTIONS].sort((a, b) => scoreSection(b, queryNormalized) - scoreSection(a, queryNormalized));
    const size = Math.max(3, Math.min(maxSections, 10));
    return sorted.slice(0, size);
}
async function fetchNotebookKnowledge(query) {
    try {
        ...
        const scored = snapshot.docs.map((doc) => {
            ...
            for (const keyword of keywords) {
                if (queryNormalized.includes(normalizeText(keyword)))
                    score += 25;
            }
            return { data, score };
        });
        ...
        const text = topSections
            .map((s) => `- [${s.data.notebook}] ${s.data.title}: ${s.data.body}`)
            .join("\n");
        if (!text.trim())
            return null;
...
async function fetchExternalClinicalContext(query) {
        ...
        try {
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    ...
                    knowledgeVersion: exports.PESSY_CLINICAL_KNOWLEDGE_VERSION,
                    topK: 5,
                }),
            });
            if (!response.ok)
            ...
            if (contentType.includes("application/json")) {
                ...
                const text = (typeof payload.context === "string" && payload.context) ||
...
async function resolveClinicalKnowledgeContext(params) {
    var _a;
    const query = (params.query || "").trim();
    const selected = pickKnowledgeSections(query, (_a = params.maxSections) !== null && _a !== void 0 ? _a : 7);
    ...
    const localBlock = selected
        ...
        .join("\n");
    const externalBlock = (external === null || external === void 0 ? void 0 : external.text)
        ? `\n\nCONTEXTO CLÍNICO EXTERNO (${external.source}):\n${external.text.slice(0, 5000)}`
        : "";
    const notebookBlock = (notebook === null || notebook === void 0 ? void 0 : notebook.text)
    ...
    const contextText = [
        ...
        `version: ${exports.PESSY_CLINICAL_KNOWLEDGE_VERSION}`,
